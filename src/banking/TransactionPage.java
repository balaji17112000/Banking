...
public void displayTransactionPage(Users customer, long id,long accNo) {
	...
	case 3:
		try {
			...
			int amount = getInt();
			boolean status = trans.withdrawModey(id,accNo, amount);
			if(status) {
				System.out.println("Withdraw request submitted! waiting for Admin approval");
			}
		} catch (KeyException e) {
			LOG.info(e.getMessage());
		}
